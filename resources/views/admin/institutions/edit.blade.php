...
@section('content')
    <div class="max-w-2xl">
        <div class="mb-6">
            <h2 class="text-lg sm:text-xl font-extrabold text-text tracking-tight">Editar Instituição</h2>
            <p class="text-sm text-text-3 mt-0.5">Alterar dados de {{ $institution->name }}.</p>
        </div>

        <form method="POST" action="{{ route('admin.institutions.update', $institution->uuid) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="bg-surface-card backdrop-blur-xl border border-glass-border rounded-xl shadow-card p-4 sm:p-6 space-y-5">
                ...
                <div>
                    ...
                    <input id="commercial_name" type="text" name="commercial_name" value="{{ old('commercial_name', $institution->commercial_name) }}"
                        class="w-full px-3 py-2.5 bg-black/3 border border-black/10 rounded-lg text-sm text-text placeholder:text-text-3 focus:outline-none focus:border-blue/50 focus:bg-blue/2 transition-all">
                ...
                <div>
                    ...
                    <input id="tax_id" type="text" name="tax_id" value="{{ old('tax_id', $institution->tax_id) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="institution_type" name="institution_type" required
                            ...
                            <option value="LDA" {{ old('institution_type', $institution->institution_type) === 'LDA' ? 'selected' : '' }}>LDA</option>
                            <option value="SA" {{ old('institution_type', $institution->institution_type) === 'SA' ? 'selected' : '' }}>SA</option>
                            <option value="ENI" {{ old('institution_type', $institution->institution_type) === 'ENI' ? 'selected' : '' }}>ENI</option>
                            <option value="ONG" {{ old('institution_type', $institution->institution_type) === 'ONG' ? 'selected' : '' }}>ONG</option>
                            <option value="EP" {{ old('institution_type', $institution->institution_type) === 'EP' ? 'selected' : '' }}>EP</option>
                            <option value="OUTRO" {{ old('institution_type', $institution->institution_type) === 'OUTRO' ? 'selected' : '' }}>OUTRO</option>
                        </select>
                ...
                <div>
                    ...
                    <input id="website" type="url" name="website" value="{{ old('website', $institution->website) }}"
                        class="w-full px-3 py-2.5 bg-black/3 border border-black/10 rounded-lg text-sm text-text placeholder:text-text-3 focus:outline-none focus:border-blue/50 focus:bg-blue/2 transition-all">
                    @error('website')
                        <p class="mt-1.5 text-xs text-red">{{ $message }}</p>
                    @enderror
